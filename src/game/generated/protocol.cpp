...
int CNetObjHandler::ValidateObj(int Type, void *pData, int Size)
{
	...
	case NETOBJTYPE_FLAG:
	{
		...
		if(sizeof(*pObj) != Size) return -1;
		ClampInt("m_Team", pObj->m_Team, 0, 1);
	...
	case NETOBJTYPE_PLAYERINFO:
	{
		...
		ClampInt("m_Team", pObj->m_Team, -1, 1);
		return 0;
	}
...
void *CNetObjHandler::SecureUnpackMsg(int Type, CUnpacker *pUnpacker)
{
	...
	case NETMSGTYPE_SV_CHAT:
	{
		...
		pMsg->m_Cid = pUnpacker->GetInt();
		pMsg->m_pMessage = pUnpacker->GetString();
		if(pMsg->m_Team < -1 || pMsg->m_Team > 1) { m_pMsgFailedOn = "m_Team"; break; }
		if(pMsg->m_Cid < -1 || pMsg->m_Cid > MAX_CLIENTS-1) { m_pMsgFailedOn = "m_Cid"; break; }
	} break;
	...
	case NETMSGTYPE_SV_EMOTICON:
	{
		...
		if(pMsg->m_Cid < 0 || pMsg->m_Cid > MAX_CLIENTS-1) { m_pMsgFailedOn = "m_Cid"; break; }
	...
	case NETMSGTYPE_SV_VOTEOPTION:
	{
		...
		(void)pMsg;
		pMsg->m_pCommand = pUnpacker->GetString();
	} break;
	...
	case NETMSGTYPE_CL_STARTINFO:
	{
		...
		pMsg->m_pSkin = pUnpacker->GetString();
	...
	case NETMSGTYPE_CL_CALLVOTE:
	{
		...
		(void)pMsg;
		pMsg->m_Type = pUnpacker->GetString();
		pMsg->m_Value = pUnpacker->GetString();
	} break;
